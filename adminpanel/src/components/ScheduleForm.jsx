...
const ScheduleForm = () => {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      e.preventDefault();
      try {
        const filteredSchedule = Object.entries(formData.schedule).reduce(
          (acc, [day, classes]) => {
            if (classes.length > 0) {
              acc[day] = classes;
            }
        ...
        for (const day in filteredSchedule) {
          for (const cls of filteredSchedule[day]) {
            if (
              cls.Class_type !== "Free" &&
              (!cls.Course_Name || !cls.Instructor || !cls.Room)
            ) {
              toast.error(
                `Course Name, Instructor, and Room are required for non-free classes.`
              );
              return;
            }
          }
